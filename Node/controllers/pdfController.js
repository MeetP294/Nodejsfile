  ...
  try {
    ...
    for  (let file of Object.entries(data.PackageDetails)) {
      ...
      try {
        ...
        const uint8Array = fs.readFileSync(`${tempPdfFilesDirectoryPath}/${pdfFileName}`);
        ...
        console.log(`Storing Merge File - ${tempPdfFilesDirectoryPath}/${pdfFileName}`);
        fs.writeFileSync(pathToPdf, mergedPdfFile);

        fs.unlinkSync(`${tempPdfFilesDirectoryPath}/${pdfFileName}`);
        console.log(`Deleted Merged temproray file - ${tempPdfFilesDirectoryPath}/${pdfFileName}`);
